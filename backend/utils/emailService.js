...
const sendVnpaySuccessEmail = async (to, order) => {
  ...
  const mailOptions = {
    from: `"XPhone" <${process.env.EMAIL_USER}>`,
    to,
    subject: `Đặt hàng thành công - Đơn ${order.orderCode}`,
    html: `
      <div style="font-family: Arial, sans-serif; line-height: 1.6;">
        <h2>Chào ${customerName},</h2>
        <p style="font-size: 15px;">Cảm ơn bạn đã thanh toán thành công qua VNPAY!</p>
        <p>Mã đơn hàng: <strong>${order.orderCode}</strong></p>
        <p>Thời gian: ${orderDate}</p>
        <p><strong>Đơn hàng của bạn đang được xử lý và chuẩn bị giao.</strong></p>
        ...
        <p style="font-size: 15px;">XPhone cảm ơn bạn rất nhiều!</p>
      </div>
    `,
  };
