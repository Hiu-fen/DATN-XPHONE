...
const MenuNgangHeader = () => {
  ...
  const { data: categories, isLoading: categoriesLoading, isError: categoriesError } = useQuery<ICategory[]>({
    queryKey: ["categories"],
    queryFn: async () => {
  ...
  const { data: products, isLoading: productsLoading, isError: productsError } = useQuery<IProduct[]>({
    queryKey: ["products"],
  ...
  // Hàm render submenu theo loại submenuType
    ...
    if (item.submenuType === "categories") {
      ...
      return (
        <div className="w-full">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Danh mục nổi bật</h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-8">
            {categories.map((cat) => (
              ...
              >
                <div className="w-full max-w-[160px] mx-auto aspect-square overflow-hidden rounded-lg border border-gray-200 group-hover:shadow-md transition">
                  <img
                    src={cat.image}
                    alt={cat.name}
  ...
  return (
    <nav className="bg-gray-100 border-t relative z-50 w-full">
      <div
        className="relative"
      ...
      >
        ...
        {renderedSubmenu && (
          <div className="absolute left-0 right-0 top-full z-40 pointer-events-none">
            <div className="w-full h-[700px] bg-black/50 transition duration-300 ease-in-out"></div>
          </div>
        )}

        {/* Submenu */}
        {renderedSubmenu !== null && (
          <div className="absolute left-0 w-full bg-white shadow-xl border-t z-40 animate-slideDown">
