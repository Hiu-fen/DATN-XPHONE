...
const Details = () => {
  ...
  const isHighValueOrder = (unitPrice: number, quantity: number): boolean => {
    ...
    const threshold = 200000000; // 200 triệu 
  ...
  // 🔥 FUNCTION XỬ LÝ MUA HÀNG HOẶC THÊM GIỎ HÀNG
  const handleAction = (action: "buyNow" | "addToCart") => {
    if (!product) {
    ...
    // 🔥 KIỂM TRA TỔNG GIÁ TRỊ ĐƠN HÀNG CHỈ KHI MUA NGAY
    if (action === "buyNow") {
      const unitPrice = getCurrentPrice();
