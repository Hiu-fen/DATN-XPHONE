...
const OrderDetail = () => {
  ...
  // same logic as Checkout: compute orderDiscount based on completed orders count
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        ...
        // If orderDiscount not present or zero, try to compute it using user's completed orders
        // and persist to backend so next time it's available directly.
        if ((!orderData.orderDiscount || orderData.orderDiscount === 0) && orderData.userId) {
          try {
            const res = await axios.get(
              `http://localhost:5000/api/orders/user/${orderData.userId}?status=Hoàn thành`,
            ...
            if (computed > 0) {
              ...
              try {
                await axios.patch(`http://localhost:5000/api/orders/${orderData._id}`, {
                  orderDiscount: computed,
                })
              } catch (patchErr) {
                // non-fatal: log and continue
                console.error("Không thể lưu orderDiscount lên server:", patchErr)
              }
            }
          } catch (err) {
            // don't block display if this fails; just log
            console.error("Không thể lấy số đơn hoàn thành để tính giảm giá thành tích:", err)
          }
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const handleOrderUpdated = (updatedOrder: Order) => {
      if (updatedOrder._id === id) {
        ...
        // same logic: if updatedOrder lacks orderDiscount, try to compute and persist
        if ((!updatedOrder.orderDiscount || updatedOrder.orderDiscount === 0) && updatedOrder.userId) {
          (async () => {
            try {
              const res = await axios.get(
                `http://localhost:5000/api/orders/user/${updatedOrder.userId}?status=Hoàn thành`,
                { withCredentials: true },
              )
              const completedCount = Array.isArray(res.data) ? res.data.length : 0
              const computed = getOrderDiscount(completedCount)
              if (computed > 0) {
                setOrder((prev) => (prev ? { ...prev, orderDiscount: computed } : prev))
                try {
                  await axios.patch(`http://localhost:5000/api/orders/${updatedOrder._id}`, {
                    orderDiscount: computed,
                  })
                } catch (patchErr) {
                  console.error("Không thể lưu orderDiscount lên server:", patchErr)
  ...
  // -----------------------
  // compute totals including orderDiscount & voucherDiscount & refunds
  // -----------------------
  const computeOrderTotals = (o: Order | null) => {
    ...
    // If backend stored originalTotal -> use it.
    // Otherwise compute from components: subtotal - orderDiscount - voucherDiscount + shippingFee
  ...
  return (
    <div className="min-h-screen mx-4 border-t border-gray-200 bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
        ...
        {/* Info cards & details (kept intact from your original UI) */}
